...
# sqrt_out = zero_sqrt_out(Es,Bs,s,self.Rinnm,self.UinmeV,self.VinmeV,self.tinmeV,tau)
	...
	def dets_calc(self):
		...
		te = time.time()
		print('determinant calculation finished. Ellapsed time: {:.4f}\n'.format(te-ts,'\n'))
	...
	def search_minima(self):
		...
		return BinTs_resolved,EinmeVs_resolved,all_minima
	...
	def calc_lines(self):
		...
		BinTs_resolved,EinmeVs_resolved,all_minima = self.search_minima()

		for i in range(len(all_minima[0])):
			# print(all_minima[0])
			...
			if det(E_t(all_minima[0][i],self.Rinnm),self.s,r_t(BinTs_resolved[0],self.Rinnm),self.m,U0_t(self.UinmeV,self.Rinnm),V_t(self.VinmeV,self.Rinnm),t_t(self.tinmeV,self.Rinnm),self.tau)<0.006:
				all_minima_sorted.append([all_minima[0][i]])
		for i in range(1,len(all_minima)):
			for j in range(len(all_minima[i])):
				for k in range(len(all_minima_sorted)):
					if abs(all_minima[i][j]-all_minima_sorted[k][-1])<1.0:
		...
		return BinTs_resolved,EinmeVs_resolved,all_minima_sorted_valid
